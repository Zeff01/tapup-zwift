...
import { doc, setDoc } from "@/lib/firebase/firestore-monitored";
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `${numberOfCards} blank cards created with IDs: ${createdCardIds.join(", ")}`
    );

    return NextResponse.json(
      {
        message: `${numberOfCards} cards created successfully`,
        createdCardIds,
      },
      { status: 201 }
    );
