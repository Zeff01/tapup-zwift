...
async function deleteAllDocumentsInCollection(collectionName: string) {
  ...
  try {
    ...
    while (totalDeleted < snapshot.size) {
      ...
      snapshot.docs.slice(totalDeleted, totalDeleted + batchSize).forEach((doc) => {
...
async function verifyCollectionEmpty(collectionName: string) {
  ...
  if (!snapshot.empty) {
    console.log(`  ⚠️  WARNING: ${collectionName} still has ${snapshot.size} documents!`);
    return false;
  }
...
async function main() {
  ...
  console.log('================================\n');
  console.log('⚠️  CRITICAL WARNING: This will DELETE ALL DATA from ALL collections!');
  console.log('This includes: orders, transactions, pregenerated cards, and more!');
  ...
  try {
    ...
    console.log(`Will attempt to clear ${collectionsToClean.length} collections:\n`);
