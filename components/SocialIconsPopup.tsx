...
const SocialIconsPopup: React.FC<SocialIconsPopupProps> = (props) => {
    ...
    const availableIcons = (Object.entries(ICON_MAP) as [keyof SocialIconsPopupProps, IconEntry][])
        .filter(([key]) => {
    ...
    return (
        <>
            <div className="absolute top-52 grid grid-cols-1 place-items-center gap-6 ml-6">
                {visible}
                {hidden.length > 0 && (
                    <button onClick={() => setIsModalOpen(true)} aria-label="More social icons" className='-mt-2'>
                        <FaEllipsis size={18} />
                    </button>
                )}
            </div>
