...
export const getCartByUserUid = async (userUid: string | undefined): Promise<cartData | null> => {
    try {
...
export const saveCartItemsByUserUid = async (userUid: string, cartItems: CartItem[]) => {
    try {
        const cartCollection = collection(firebaseDb, "cart");
        const q = query(cartCollection, where("userUid", "==", userUid));
        const cartSnap = await getDocs(q);
